...
PixelDraw g_pixelDraw(g_canvasWidth, g_canvasHeight, g_pixelBlockSize, g_canvasPixels,g_layerVector[0].mainLayerPixels, g_layerVector[0].tmpLayerPixels, &g_mainCanvasTexture);
...
void CreateCanvas(int pixelSize){
  ...
  g_pixelDraw = PixelDraw(g_canvasWidth, g_canvasHeight, 
      ...
      g_layerVector[g_selectedLayerIndex].tmpLayerPixels, &g_mainCanvasTexture);
  CenterCanvasPos();
...
int main(void) {
  ...
  GUIControl guiControl = GUIControl{&g_pixelDraw, g_screenWidth, g_screenHeight, &g_canvasPos, &g_canvasWidth, &g_canvasHeight};
  ...
  while (!WindowShouldClose()) // Detect window close button or ESC key
  {
    ...
    if (IsMouseButtonDown(MOUSE_BUTTON_LEFT) && !guiControl.io.WantCaptureMouse) {
